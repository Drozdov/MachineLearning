...
public class SampleXYDataset extends AbstractXYDataset 
    ...
    public SampleXYDataset(int seriesCount, Double[]... values) {
        ...
        this.yValues = new Double[seriesCount][];
    	...
    	for (int i = 0; i < seriesCount; i++) {
    		this.xValues[i] = values[2 * i];
    		this.yValues[i] = values[2 * i + 1];
    		this.itemCount += values[2 * i].length;
    		for (double x : xValues[i]) {
    			if (x < minX)
    				minX = x;
    			if (x > maxX)
    				maxX = x;
    		}
    		for (double y : yValues[i]) {
    			if (y < minY)
    				minY = y;
    			if (y > maxY)
    				maxY = y;
